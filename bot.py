...
def parse_filename(filename: str) -> Dict[str, str]:
    ...
    # (2021) jaisa year dhoondein
    match = re.search(r"\(((19|20)\d{2})\)", filename)
    if match:
        year = match.group(1)
    else:
        # Agar na mile, toh aakhri 4-digit number dhoondein
        matches = re.findall(r"\b((19|20)\d{2})\b", filename)
        if matches:
            year = matches[-1] # Aakhri wala chunein
    
    # Extension hatayein (.mkv, .mp4, etc.)
...
async def get_movie_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        # YEH FALLBACK JSON IMPORT KE LIYE CRITICAL HAI
...
@handler_timeout(1800) # 30 minute timeout for large imports
async def import_json_command(message: types.Message):
    ...
    if not progress_msg:
        return # Agar message send nahi hua toh aage nahi badh sakte

    for i, item in enumerate(movies):
        try:
            file_id = item.get("file_id")
            filename = item.get("title") # Yeh filename hai
            
            if not file_id or not filename:
                skipped += 1
                continue
            
            # 1. Unique ID generate karein
            imdb_id = f"json_{hashlib.md5(file_id.encode()).hexdigest()}"
            
            # 2. Check karein ki pehle se hai ya nahi
            existing = await safe_db_call(db.get_movie_by_imdb(imdb_id), timeout=3)
            if existing:
                skipped += 1
                continue
                
            # 3. Title aur Year parse karein
            parsed_info = parse_filename(filename)
            
            # 4. Database mein add karein
            success = await safe_db_call(db.add_movie(
                imdb_id=imdb_id,
                title=parsed_info["title"],
                year=parsed_info["year"],
                file_id=file_id,
                message_id=AUTO_MESSAGE_ID_PLACEHOLDER, # Placeholder istemaal karein
                channel_id=0 # Placeholder istemaal karein
            ), default=False)
        ...
        # 5. Progress update dein (Free Tier ke liye zaroori)
        if (i + 1) % 50 == 0 or (i + 1) == total:
            ...
            await asyncio.sleep(0.5) # Event loop ko block hone se bachayein
