...
# Flask related imports are removed to eliminate threading conflict
# from flask import Flask, jsonify 
# from threading import Thread # Threading is removed
...
# WEB_SERVER_PORT is no longer needed but kept for completeness
...
async def handle_channel_post(message: Message):
    try:
        ...
        if message.document or message.video:
            ...
            # FIX: Caption se title nikalein
            title = caption.split('\n')[0].strip() if caption else message.message_id 
            post_id = message.message_id 
            
            if title and post_id:
                # FIX: Indexing ab duplicate check ke saath chalega
...
async def cmd_broadcast(message: Message):
    if not message.from_user or message.from_user.id not in ADMIN_IDS: 
        return
    # Broadcast logic remains the same (assumes image/video logic works fine)
